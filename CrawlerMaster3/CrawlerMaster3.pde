...
double target_lats[NUMBER_OF_WAYPOINTS] = {37.54683, 37.54720}; 
double target_lons[NUMBER_OF_WAYPOINTS] = {122.30217, 122.30225};  
  ...
  */
  ...
  switch (mode) {
    ...
    case GPS_MODE:
      ...
      if(navigationDistance < 5 && navigationDistance != 0) {
        mode = CAMERA_MODE;
        break;
      }
    ...
    case CAMERA_MODE: {
      ...
      if(spiralCounter < 80) {
        spiralCounter++;
        steerServo.write(180 - spiralCounter*90/80.0 );
        pwrServo.write(LOW_POWER);
        delay(500);
        Serial.println(180-spiralCounter*90/80.0);
        break;
      }
        spiralCounter = 0;
        mode = OBSTACLE_MODE;
