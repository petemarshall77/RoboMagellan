  ...
  */
  ...
  switch (mode) {
    ...
    case GPS_MODE:
      ...
      if(navigationDistance < 6 && navigationDistance != 0) {
        cameraModeRetryCount = 0;
        mode = CAMERA_MODE;
        break;
      }
    ...
    case CAMERA_MODE: {
      ...
      boolean cmu1 = digitalRead(12) ? HIGH : LOW;
      boolean cmu2 = digitalRead(11) ? HIGH : LOW;
   
      if(!cmu1 && !cmu2) {
        Serial.println("CMU1 = LOW, CMU2 = LOW");
        if (cameraModeRetryCount < 2) {
          cameraModeRetryCount++;  
          steerServo.write(LEFT);
          pwrServo.write(BACKWARD);
          for (int i = 0; i < 200; i++) {
            cmu1 = digitalRead(12) ? HIGH : LOW;
            cmu2 = digitalRead(11) ? HIGH : LOW;
            if (cmu1 || cmu2) {
              cameraModeRetryCount = 0;
              stopRobot();
              break;
        ...
        Serial.println("CMU1 = LOW, CMU2 = HIGH");
        steerServo.write(LEFT);
        pwrServo.write(LOW_POWER);
      }
      else if(cmu1 && !cmu2) {
        Serial.println("CMU1 = HIGH, CMU2 = LOW"); 
        steerServo.write(RIGHT);
        pwrServo.write(LOW_POWER);
      }  
      else if(cmu1 && cmu2) {
        Serial.println("CMU1 = HIGH, CMU2 = HIGH");
        steerServo.write(STRAIGHT);
        pwrServo.write(LOW_POWER);
      }
      break;
